...
class FlameKeeper {
  ...
  async checkState() {
    ...
    let lockoutDate = new Date(
      lastEditDate.getTime() + this.lockoutTimeMilliseconds
    );
    let ghostDate = new Date(
      lastEditDate.getTime() + this.ghostTimeoutMilliseconds
    );
    ...
    if (sinceLastEditMilliseconds < this.lockoutTimeMilliseconds) {
      if (update)
        this.logger.info(
          `FlameKeeper: composer locked out until ${lockoutDate.toISOString()} (last edit on ${lastEditDate.toISOString()})`
        );
      this.locked = true;
      ...
      if (update)
        this.logger.info(
          `FlameKeeper: composer edit window open until ${ghostDate.toISOString()} (last edit on ${lastEditDate.toISOString()})`
        );
      this.locked = false;
    } else {
      // Ghost edit
  ...
  async makeRandomEdit() {
    ...
    if (
      ...
      state.currentState.audio.length > 0
    ) {
      ...
      this.logger.info(
        `FlameKeeper: composer window closed without edit, making ghost edit for: ${activeComposerName} (${activeComposerId})`
      );
      let randomAudio =
        state.audio[Math.floor(Math.random() * state.audio.length)];
