...
const isLoginActive = function (req) {
  ...
  let authenticatedID =
    authenticatedUser.composerID || authenticatedUser.adminID;
  if (Composers.isActive(authenticatedID) || Admins.isActive(authenticatedID))
    return true;
  else return false;
};
    ...
    await new Promise((resolve, reject) => {
      new IncomingForm().parse(req, async (err, fields, files) => {
        ...
        try {
          ...
          let audioID = await state.addAudio(
            filename,
            file.path,
            authenticatedID
          );
